//app.js
//几个问题，比如数据库的用户的order已经被更新了，但是我无法实时更新
const AV = require('./libs/av-core-min.js');
var plugin = requirePlugin("chatbot");
const adapters = require('./libs/leancloud-adapters-weapp.js');
AV.setAdapters(adapters);
AV.init({
  //去到leancloud-设置——应用keys改成自己的appId、appKey
  appId: 'hkx6XcikpyoJGRVUOGlJvHzr-gzGzoHsz',
  appKey: 'hNEez5tSkRDAdfvF0TwECjmX',
  // 请将 xxx.example.com 替换为你的应用绑定的自定义 API 域名
  serverURLs: "https://hkx6xcik.lc-cn-n1-shared.com",
});
...
App({
  ...
  onLaunch: function () {
    ...
    // const userAccount = AV.User.current()
    // this.globalData.patientName = userAccount.toJSON().name
    // this.globalData.userId = userAccount.id

    // if (userAccount.toJSON().phoneVerified != true) {
    //   userAccount.set('phoneVerified', false)
    //   userAccount.save()
    // }
          ...
          wx.getUserInfo({
            success: res => {
              ...
              plugin.init({
                ...
                fail: (error) => {console.log('出书画成功1') }, //非必填
  ...
  globalData: {
    ...
    date: '',
    period: '',
    //这个数据是用来确定用户是否有通过手机验证，我无法做到刷新用户记录，所以用这个值
    phoneVerified: false,
    orderId: '',
    orderIndex: '',
